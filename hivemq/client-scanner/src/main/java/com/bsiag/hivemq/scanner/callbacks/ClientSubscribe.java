 ...
 */
public class ClientSubscribe implements OnSubscribeCallback {

    Logger log = LoggerFactory.getLogger(ClientSubscribe.class);
    
	private final PublishService service;
	private final RetainedMessageStore store;

    @Inject
    public ClientSubscribe(final PublishService publishService, final RetainedMessageStore retainedMessageStore) {
	...
	public void onSubscribe(SUBSCRIBE message, ClientData clientData) throws InvalidSubscriptionException {
        ...
        log.info("Client {} subscribes topics: {}", clientId, Joiner.on(",").join(topics));

        Scanner.publishTopics(service, store, clientId, topics, new ArrayList<String>());
    }

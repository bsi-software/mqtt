...
public class Scanner {
    ...
    public static void publishOnline(PublishService service, RetainedMessageStore store, String clientId) {
        ...
        if(getInstance().subscribedTopics.get(clientId) == null) {
        	getInstance().subscribedTopics.put(clientId, new HashSet<String>());	
        }
     	
        if(getInstance().unsubscribedTopics.get(clientId) == null) {
        	getInstance().unsubscribedTopics.put(clientId, new HashSet<String>());	
        }
    }
    ...
    private static String getTopicSubscribed(String clientId) {
    	StringBuffer topic = new StringBuffer(Scanner.HIVEMQ_SCANNER);
    	topic.append(clientId);
    	topic.append(Scanner.SUBSCRIBED);
    	return topic.toString();
    }
    
    private static String getTopicUnubscribed(String clientId) {
    	StringBuffer topic = new StringBuffer(Scanner.HIVEMQ_SCANNER);
    	topic.append(clientId);
    	topic.append(Scanner.UNSUBSCRIBED);
    	return topic.toString();
    }
	...
	public static void publishTopics(PublishService service, RetainedMessageStore store, String clientId, 
			List<Topic> subscribeTopics, List<String> unsubscribeTopics) 
	{
		...
		for(Topic topic: subscribeTopics) {
			subscribedTopics.add(topic.getTopic());
			unsubscribedTopics.remove(topic.getTopic());
		}
		
		for(String topic: unsubscribeTopics) {
			subscribedTopics.remove(topic);
			unsubscribedTopics.add(topic);
		}
		
    	PUBLISH message = createMessage(getTopicSubscribed(clientId), Joiner.on("\n").join(subscribedTopics));
    	handleMessage(service, store, message);
		
    	message = createMessage(getTopicUnubscribed(clientId), Joiner.on("\n").join(unsubscribedTopics));
    	handleMessage(service, store, message);		
	}
}
